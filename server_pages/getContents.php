...
if(isset($_GET['mode'])){
	...
	switch ($mode) {
		case 'recommendedVideos':
			$query = mysqli_query($conn, "select * from files where file_type like '%video%' order by rand() limit 25")or die(mysqli_error($conn));
			...
			case 'trendingVideos':
				echo "Trenving Viveos";
				break;

			case 'mostWatchedVideos':
				echo "Most Watched Videos";
				break;

		    case 'recommendedMusic':
			$query = mysqli_query($conn, "select * from files where file_type like '%audio%' order by rand() limit 25")or die(mysqli_error($conn));
			while ($row = mysqli_fetch_array($query)) {
				$file_name = $row['file_name'];
				$file_owner = $row['file_owner'];
				$file_type = $row['file_type'];
